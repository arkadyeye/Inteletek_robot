...
void InitEnd(ConfigData *pTheConfigData)
{
    ...
    if (pTheConfigData) {
        std::cout << "  Mode: " << pTheConfigData->m_sMode
                  << "  System Type: " << pTheConfigData->m_sSystemType
                  << "  Axes: " << pTheConfigData->m_sNumOfAxes
                  << std::endl;

        std::wcout << L"  Software Version: " << pTheConfigData->m_szUSBC_SoftVer << L"\n";

        std::wcout << L"  Device IDs:\n";
        for (int i = 0; i < TOTAL_AXES; ++i) {
            if (pTheConfigData->m_szDeviceID[i][0] != 0)
                std::wcout << L"    Axis " << i << L": " << pTheConfigData->m_szDeviceID[i] << L"\n";
        }
    }
}
