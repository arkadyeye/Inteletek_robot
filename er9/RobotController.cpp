...
bool RobotController::getConfig() {
    ...
    std::cout << "[RobotController] Configuration:\n";
    std::cout << "  Mode: " << cfg.m_sMode << "\n";
    std::cout << "  System Type: " << cfg.m_sSystemType << "\n";
    std::cout << "  Number of Axes: " << cfg.m_sNumOfAxes << "\n";
    std::cout << "  Gripper Type: " << cfg.m_sGripperType 
              << " (Range: " << cfg.m_sGripperRange << " mm)\n";
    ...
    std::wcout << L"  Software Version: " << cfg.m_szUSBC_SoftVer << L"\n";

    std::wcout << L"  Device IDs:\n";
    for (int i = 0; i < TOTAL_AXES; i++) {
        if (cfg.m_szDeviceID[i][0] != 0) {
            std::wcout << L"    Axis " << i << L": " << cfg.m_szDeviceID[i] << L"\n";
        }
    }
